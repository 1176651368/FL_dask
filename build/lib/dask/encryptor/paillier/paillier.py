...
from .util import invert, powmod, mulmod, getprimeover, isqrt
...
class EncryptedNumber(object):
    ...
    def __init__(self, public_key, ciphertext, exponent=0):
        ...
        self.ndim = len(self.exponent.shape)
        self.shape = self.exponent.shape
        self.dtype = float
...
def toArray(cip:EncryptedNumber):
    public_key = cip.public_key
    ciphertext = cip.ciphertext
    exponent = cip.exponent
    print(public_key,ciphertext,exponent)
    r = da.frompyfunc(lambda x,y,z:EncryptedNumber(x,y,z),3,1)(public_key,ciphertext,exponent)
    return r
