...
def partial_reduce(
    ...
    if np.isscalar(meta):
        ...
        return Array(graph, name, out_chunks, dtype=dtype, public_key=x.public_key, exponent=x.exponent,
                     ciphertext=x.ciphertext)
    else:
        with contextlib.suppress(AttributeError):
            meta = meta.astype(dtype)
        return Array(graph, name, out_chunks, meta=meta, public_key=x.public_key, exponent=x.exponent,
                     ciphertext=x.ciphertext)
